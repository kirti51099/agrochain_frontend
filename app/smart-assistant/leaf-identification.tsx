...
//       const res = await fetch(`${BACKEND_BASE}/api/images/upload`, {
//         method: "POST",
//         body: form as any,
...
//       if (!res.ok) {
//         setUploading(false);
//         const errorMsg = (json && json.message) || `Upload failed (${res.status})`;
//         setError(errorMsg);
...
//   // Fix slice error: ensure we always have an array before calling slice
//   const suggestionsArray = record?.apiResponse?.suggestions || 
//                            record?.apiResponse?.data?.suggestions || 
//                            record?.suggestions || 
//                            [];
  
//   const suggestions = Array.isArray(suggestionsArray) && suggestionsArray.length > 0
//     ? suggestionsArray.slice(0, 3).map((s: any, idx: number) => ({
//         id: `${idx}-${s.plant_name || (s.plant && s.plant.scientific_name) || "Unknown"}`,
//         name: s.plant_name || (s.plant && s.plant.scientific_name) || "Unknown",
//         confidence: typeof s.probability === "number" ? Math.round(s.probability * 100) : 
//                    (typeof s.score === "number" ? Math.round(s.score * 100) : null),
//         wiki: (s.plant && s.plant.wiki_description && s.plant.wiki_description.value) || 
//               (s.plant && s.plant.wiki_description) || 
//               null,
//       }))
//     : [];
...
//               <Text style={styles.label}>🌿 Plant Identification Results</Text>
//               {suggestions.length === 0 && (
//                 <View style={styles.noResults}>
//                   <Text style={styles.muted}>No suggestions yet. Try a clearer leaf photo.</Text>
//                   <Text style={[styles.muted, { marginTop: 4, fontSize: 12 }]}>
//                     Make sure the leaf is clear, well-lit, and in focus.
//                   </Text>
//                 </View>
//               )}
...
//                     {s.confidence && (
//                       <Text style={styles.confidenceLabel}>Confidence: {s.confidence}%</Text>
//                     )}
//                   </View>
//                   <View style={styles.confidenceBadge}>
//                     <Text style={styles.suggestConf}>{s.confidence ? `${s.confidence}%` : "-"}</Text>
//                   </View>
//                 </View>
//               ))}
//             </View>

//             {suggestions[0] && suggestions[0].wiki ? (
//               <View style={styles.wikiBox}>
//                 <Text style={styles.wikiTitle}>📖 About {suggestions[0].name}</Text>
//                 <Text style={styles.wikiText}>{suggestions[0].wiki}</Text>
//               </View>
//             ) : null}
...
//               {suggestions.length > 0 && (
//                 <TouchableOpacity 
//                   style={[styles.viewBtn, { backgroundColor: "#10B981" }]} 
//                   onPress={() => {
//                     const topPlant = suggestions[0]?.name || "Unknown";
//                     Alert.alert(
//                       "Get Pesticide Recommendations",
//                       `Would you like to get pesticide recommendations for ${topPlant}?`,
//                       [
//                         { text: "Cancel", style: "cancel" },
//                         { 
//                           text: "Yes", 
//                           onPress: () => router.push("/smart-assistant/pesticide-recommendation")
//                         }
//                       ]
//                     );
//                   }}
//                 >
//                   <MaterialCommunityIcons name="bug-check" size={18} color="#fff" />
//                   <Text style={styles.viewBtnText}>Get Pesticide Recommendations</Text>
//                 </TouchableOpacity>
//               )}
...
//                 } else {
//                   Alert.alert("Info", "Image URL not available");
//                 }
//               }}>
...
//   label: { fontWeight: "700", marginBottom: 6, fontSize: 16, color: "#1b4332" },
//   noResults: { padding: 12, backgroundColor: "#fef3c7", borderRadius: 8, borderLeftWidth: 4, borderLeftColor: "#f59e0b" },
//   suggestion: { 
//     flexDirection: "row", 
...
//     paddingHorizontal: 8,
//     borderBottomWidth: 1, 
//     borderBottomColor: "#e5f5e5",
//     backgroundColor: "#f9faf9",
//     borderRadius: 8,
//     marginBottom: 8
//   },
//   suggestName: { fontWeight: "800", color: "#0f5132", fontSize: 15 },
//   confidenceLabel: { fontSize: 12, color: "#6b7280", marginTop: 2 },
//   confidenceBadge: {
//     backgroundColor: "#2f8f73",
...
//     alignItems: "center"
//   },
//   suggestConf: { color: "#fff", fontWeight: "800", fontSize: 13 },
...
// });
//app/smart-assistant/leaf-identification.tsx
// import { Feather, FontAwesome5, MaterialCommunityIcons } from "@expo/vector-icons";
// import * as ImagePicker from "expo-image-picker";
// import React, { useEffect, useRef, useState } from "react";
// import { ActivityIndicator, Alert, Animated, Image, Platform, SafeAreaView, ScrollView, StyleSheet, Text, TouchableOpacity, View } from "react-native";
...
//         Alert.alert(
//           "Not a leaf image",
//           "This photo does not look like a plant leaf. Please try again with a clear leaf photo."
//         );
//         return;
//       }
...
//             // If backend later decides it's not a leaf
//             if (jr && (jr.isLeafImage === false || jr.diagnosis === "Not a leaf / plant image")) {
//               setPolling(false);
//               setUploading(false);
//               setProgress(0);
//               Alert.alert(
...
//   heroTitle: { fontSize: 26, color: "#fff", fontWeight: "700", textAlign: "center", marginBottom: 8 },
//   heroSubtitle: { fontSize: 17, color: "#e0e0e0", textAlign: "center", lineHeight: 24, maxWidth: "90%" },
//   actionRow: { flexDirection: "row", width: "100%", justifyContent: "space-between", marginTop: -40, marginBottom: 16 },
//   actionCard: { flex: 1, marginHorizontal: 4, backgroundColor: "#10B981", paddingVertical: 16, borderRadius: 18, alignItems: "center", shadowColor: "#000", shadowOpacity: 0.12, shadowRadius: 8, elevation: 6 },
//   actionTitle: { color: "#fff", fontSize: 15, fontWeight: "700", marginTop: 6 },
//   actionSub: { color: "#edfdf7", fontSize: 12, marginTop: 2 },
//   cta: { width: "100%", backgroundColor: "#059669", paddingVertical: 14, borderRadius: 999, marginBottom: 18 },
...
//   previewBox: { width: "100%", marginBottom: 12, borderRadius: 18, backgroundColor: "#ecfdf5", padding: 12 },
//   previewImage: { width: "100%", height: 240, borderRadius: 14 },
//   previewEmpty: { height: 200, alignItems: "center", justifyContent: "center" },
//   previewText: { marginTop: 8, color: "#6b7280" },
//   progressWrap: { width: "100%", height: 6, borderRadius: 999, backgroundColor: "#d1fae5", overflow: "hidden", marginBottom: 10 },
//   progressBar: { height: "100%", backgroundColor: "#10B981" },
//   resultCard: { width: "100%", backgroundColor: "#ffffff", padding: 16, borderRadius: 18, shadowColor: "#000", shadowOpacity: 0.06, shadowRadius: 8, elevation: 4, marginBottom: 16 },
//   resultHeading: { fontSize: 18, fontWeight: "700", color: "#064e3b", marginBottom: 8 },
//   muted: { color: "#6b7280", fontSize: 13 },
//   small: { fontSize: 13, color: "#374151", marginTop: 2 },
//   mono: { fontFamily: Platform.select({ ios: "Menlo", android: "monospace" }), fontSize: 13, color: "#111827" },
//   label: { fontSize: 15, fontWeight: "600", color: "#065f46", marginBottom: 6 },
//   noResults: { paddingVertical: 8 },
//   suggestion: { flexDirection: "row", alignItems: "center", paddingVertical: 8, borderBottomWidth: 0.5, borderBottomColor: "#e5e7eb" },
//   suggestName: { fontSize: 15, fontWeight: "600", color: "#065f46" },
//   confidenceLabel: { fontSize: 12, color: "#4b5563" },
//   confidenceBadge: { paddingHorizontal: 10, paddingVertical: 4, backgroundColor: "#dcfce7", borderRadius: 999, marginLeft: 8 },
//   suggestConf: { fontSize: 12, fontWeight: "700", color: "#047857" },
//   wikiBox: { marginTop: 12, backgroundColor: "#ecfdf5", padding: 10, borderRadius: 12 },
//   wikiTitle: { fontSize: 14, fontWeight: "700", color: "#064e3b", marginBottom: 4 },
//   wikiText: { fontSize: 13, color: "#374151", lineHeight: 20 },
//   viewBtn: { flexDirection: "row", alignItems: "center", justifyContent: "center", paddingVertical: 10, borderRadius: 999, backgroundColor: "#2563EB" },
//   viewBtnText: { color: "#fff", fontWeight: "600", marginLeft: 6 },
//   tips: { width: "100%", marginTop: 8, padding: 12, borderRadius: 14, backgroundColor: "#ecfdf3" },
//   tipTitle: { fontSize: 14, fontWeight: "700", color: "#166534", marginBottom: 6 },
//   tipRow: { flexDirection: "row", alignItems: "center", marginBottom: 4 },
//   tipText: { marginLeft: 6, fontSize: 13, color: "#374151" },
// });
...
export default function LeafIdentification() {
  ...
  async function uploadAndIdentify() {
    ...
    try {
      ...
      if (!rec) {
        setUploading(false);
        setError("No record returned from server");
        return;
      }

      setRecord(rec);

      // handle non-leaf image
      if (
        rec &&
      ...
      if (
        !rec.diagnosis ||
        rec.diagnosis === "Pending" ||
        rec.apiResponse?.status === "pending"
      ) {
        setPolling(true);
        for (let i = 0; i < 12; i++) {
          await new Promise((r) => setTimeout(r, 3000));
          try {
            const chk = await fetch(
              `${BACKEND_BASE}/api/images/${rec._id || rec.id}`
            );
            const jr = await chk.json();
            setRecord(jr);
            setProgress(0.6 + ((i + 1) / 12) * 0.35);
            ...
            if (
              jr &&
              (jr.isLeafImage === false ||
                jr.diagnosis === "Not a leaf / plant image")
            ) {
              setPolling(false);
              setUploading(false);
              setProgress(0);
            ...
            if (
              jr.diagnosis &&
              jr.diagnosis !== "Pending" &&
              jr.apiResponse?.status !== "pending"
            ) {
              setPolling(false);
              setUploading(false);
              setProgress(1);
        ...
        Alert.alert(
          "Timed out",
          "Identification is taking longer than expected."
        );
      } else {
        setProgress(1);
        setUploading(false);
  ...
  const suggestionsArray =
    record?.apiResponse?.suggestions ||
    record?.apiResponse?.data?.suggestions ||
    record?.suggestions ||
    [];
  ...
  const suggestions =
    Array.isArray(suggestionsArray) && suggestionsArray.length > 0
      ? suggestionsArray.slice(0, 3).map((s: any, idx: number) => ({
          id: `${
            idx
          }-${s.plant_name || (s.plant && s.plant.scientific_name) || "Unknown"}`,
          name:
            s.plant_name ||
            (s.plant && s.plant.scientific_name) ||
            "Unknown",
          confidence:
          ...
          wiki:
            (s.plant &&
              s.plant.wiki_description &&
              s.plant.wiki_description.value) ||
            (s.plant && s.plant.wiki_description) ||
            null,
        }))
      : [];
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.hero}>
          <Image
            source={require("../../assets/images/hero_leaf.jpg")}
            style={styles.heroImage}
          />
          <View style={styles.heroOverlay}>
            <Text style={styles.heroTitle}>Snap a leaf. Get quick AI help.</Text>
            <Text style={styles.heroSubtitle}>
              Identify plants and check their health — fast and easy.
            </Text>
          </View>
        </View>
        ...
        {/* CTA */}
        <TouchableOpacity
          style={[styles.cta, uploading && { opacity: 0.8 }]}
          onPress={uploadAndIdentify}
          disabled={uploading}
        >
          {uploading ? (
            ...
            <View style={styles.ctaInner}>
              <Text style={styles.ctaText}>Upload & Identify</Text>
            </View>
          )}
        </TouchableOpacity>
        ...
        <View style={styles.previewBox}>
          {imageUri ? (
            ...
            <View style={styles.previewEmpty}>
              <Feather name="image" size={36} color="#6b7280" />
        ...
        <View style={styles.resultCard}>
          ...
          {!record && (
            <Text style={styles.muted}>
              Nothing uploaded yet — try Upload & Identify.
            </Text>
          )}
          {record && (
            <>
              <Text style={styles.small}>
                ID: <Text style={styles.mono}>{record._id ?? record.id}</Text>
              </Text>
              <Text style={styles.small}>
                Filename:{" "}
                <Text style={styles.mono}>{record.filename}</Text>
              </Text>
              ...
              <View style={{ marginTop: 10 }}>
                ...
                {suggestions.length === 0 && (
                  <View style={styles.noResults}>
                    <Text style={styles.muted}>
                      No suggestions yet. Try a clearer leaf photo.
                    </Text>
                    <Text
                      style={[
                        styles.muted,
              ...
              {suggestions[0] && suggestions[0].wiki ? (
                <View style={styles.wikiBox}>
                  <Text style={styles.wikiTitle}>
                    📖 About {suggestions[0].name}
                  </Text>
                      ...
                      Alert.alert(
                        ...
                        [
                          { text: "Cancel", style: "cancel" },
                          {
                            ...
                            onPress: () =>
                              router.push(
                                "/smart-assistant/pesticide-recommendation"
                              ),
                          },
                        ]
                      );
        ...
        <View style={styles.tips}>
          ...
          <View style={styles.tipRow}>
            <Feather name="check-circle" size={16} color="#2f855a" />
            <Text style={styles.tipText}>
              {" "}
              Use a single leaf on plain background
            </Text>
          </View>
          <View style={styles.tipRow}>
            <Feather name="check-circle" size={16} color="#2f855a" />
          ...
          <View style={styles.tipRow}>
            ...
            <Text style={styles.tipText}>
              {" "}
              Avoid hands or clutter around leaf
            </Text>
          </View>
        </View>
